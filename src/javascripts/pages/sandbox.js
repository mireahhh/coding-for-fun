...
const sandboxRuntimeStorageKey = "sandboxRuntime";
const sandboxStates = new Set();
let isBeforeUnloadInitialized = false;

function isSandboxRuntime(runtime) {
    return sandboxRuntimeOrder.includes(runtime);
}

function getSavedSandboxRuntime() {
    const savedRuntime = localStorage.getItem(sandboxRuntimeStorageKey);

    return isSandboxRuntime(savedRuntime) ? savedRuntime : sandboxDefaultRuntime;
}

function saveSandboxRuntime(runtime) {
    if (!isSandboxRuntime(runtime)) return;

    localStorage.setItem(sandboxRuntimeStorageKey, runtime);
}
...
function initSandboxCodeBlock(controller) {
    ...
    libraryButtons.forEach((button) => {
        button.addEventListener("click", () => {
            ...
            saveCurrentCode();
            saveSandboxRuntime(nextRuntime);
...
initCodeBlocks({
    getInitialRuntime: () => getSavedSandboxRuntime(),
