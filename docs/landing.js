...
function buildLandingTagsMarquee() {
  var list = document.querySelector(".C_LandingCoverRunningTags");
  if (!list) return;
  list.innerHTML = "";
  var track = document.createElement("div");
  track.className = "C_LandingCoverRunningTagsTrack";
  var baseGroup = createGroup(tags);
  list.appendChild(baseGroup);
  var containerWidth = list.clientWidth;
  var baseWidth = baseGroup.scrollWidth;
  baseGroup.remove();
  if (!baseWidth || !containerWidth) return;
  var repeatedTags = _toConsumableArray(tags);
  var currentWidth = baseWidth;
  while (currentWidth < containerWidth * 1.5) {
    repeatedTags = repeatedTags.concat(tags);
    currentWidth += baseWidth;
  }
  var firstGroup = createGroup(repeatedTags);
  var secondGroup = createGroup(repeatedTags);
  track.appendChild(firstGroup);
  track.appendChild(secondGroup);
  list.appendChild(track);
  var shiftWidth = firstGroup.scrollWidth;
  list.style.setProperty("--landing-tags-shift", "".concat(shiftWidth, "px"));
  var pxPerSecond = 80;
  var duration = shiftWidth / pxPerSecond;
  list.style.setProperty("--landing-tags-duration", "".concat(duration, "s"));
}
var landingTagsResizeTimeout = null;
function initLandingTagsMarquee() {
  buildLandingTagsMarquee();
  window.addEventListener("resize", function () {
    clearTimeout(landingTagsResizeTimeout);
    landingTagsResizeTimeout = setTimeout(function () {
      buildLandingTagsMarquee();
    }, 100);
  });
}
